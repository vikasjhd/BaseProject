...
//MARK : GOOGLE TEXT TO SPEECH

//THIS CLASS CONTAIN CODE THAT IS USED FOR GOOGLE CLOUD TEXT TO SPEECH API
//ADDED BY VIKAS SAINI ON 24 MAY 2021
...
let APIKey = "AIzaSyAKK3vI4E-fHaqy4Hc2S8NWeVF9OMul7UA"//Please do not use this key , Ask for a paid key from client, This is from Push application project.
...
class SpeechService: NSObject, AVAudioPlayerDelegate {
    ...
    func speak(text: String, voiceType: VoiceType = .waveNetFemale, completion: @escaping () -> Void) {
        ...
        //when sound is playing , and user click another button to play new sound in between
        
        //Below code will stop user to play new sound untill the current one complete
        ...
        //this will stop player to play whatever is playing and start new one
        if self.busy {
            ...
            if error != nil {
                DispatchQueue.main.async {
                    if let currentVC = GetWindow()?.topViewController(){
                        Toast.show(message: error?.localizedDescription ?? "Something Went Wrong!", controller: currentVC)
                    }
                }
...
SpeechService.shared.speak(text: "Welcome Home", voiceType: .waveNetFemale) {
    //finished speaking
    PrintToConsole("ready to speak again")
}
*/
